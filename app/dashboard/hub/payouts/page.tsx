...
export default async function HubPayoutsPage({
  searchParams,
}: {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">Hub Payouts</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Connect Stripe to receive your 2% hub fee when group buys settle.
        </p>
      </div>
      ...
      {stripe_connect === "return" && (
        <Card className="border-emerald-200 bg-emerald-50/60">
          <CardContent className="flex items-center gap-2 p-4 text-sm text-emerald-800">
            ...
            Stripe returned you to CrowdRoast. If setup is complete, hub payouts are enabled.
          </CardContent>
        </Card>
      )}
